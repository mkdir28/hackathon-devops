...
## ADR-003: Версіонування промптів та скілів як коду (PromptOps)

### Контекст
Системні промпти (`app/prompts/`) та вміння агента (`app/skills/` у форматі `SKILL.md`) визначають логіку поведінки AI. Зміна промпту може призвести до регресії в якості роботи агента (галлюцинації, зсув тону).

### Рішення
1. Зберігати промпти та файли `SKILL.md` у Git як звичайні текстові файли.
2. Будь-яка зміна промпту або скіла повинна проходити через Pull Request.
3. У CI налаштовано якісний запуск `evals/run-evals.mjs`, який оцінює якість роботи агента при поточних промптах перед мерджем.

### Наслідки
- **Плюси:** Повна історія змін (Audit Trail), гарантія того, що невдалий промпт не потрапить на прод.
- **Мінуси:** Розробникам промптів потрібно використовувати Git та чекати проходження евалюації.
