...
* **Як працює:** На самому початку пайплайну запускається крок `Run Gitleaks Scan`, який використовує `gitleaks/gitleaks-action@v2` для глибокого аналізу всієї історії комітів поточного пушу.
...
### Б. Сканування Skills (Prompts Security Scanner)
* **Як працює:** Крок `Run Skills Security Check` запускається при кожній зміні вмісту папок `app/skills/*` та `app/prompts/*` і виконує скрипт `scripts/check-skills-security.mjs`.
* **Блокування:** Скрипт валідує структуру промптів та перевіряє наявність секретів у тексті промптів, блокуючи виконання у разі виявлення ризиків.
