...
* **Рекомендована модель:** **GPT-4o-mini** (Бюджетна) або **Claude 3.5 Sonnet** (Преміальна).
  * *Обґрунтування:* GPT-4o-mini підтримує функцію *Strict JSON Schema*, що гарантує 100% валідність результату на рівні API, будучи надзвичайно дешевою ($0.15 / 1M input tokens). Claude 3.5 Sonnet ідеально розпізнає складні кар'єрні траєкторії, але коштує дорожче ($3.00 / 1M input tokens).
...
* **Рекомендована модель:** **Gemini 2.5 Flash** (або Gemini 2.5 Flash-lite).
  * *Обґрунтування:* Вхідні токени коштують усього $0.075 за 1M токенів. Модель має величезне контекстне вікно та високу швидкість генерації, завдяки чому результати пошуку вакансій з'являються на екрані майже миттєво при мінімальних фінансових витратах.
