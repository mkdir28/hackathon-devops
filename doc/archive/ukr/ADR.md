...
### Розрахунки FinOps для інфраструктури продакшн контуру
На основі цін у регіоні `us-central1` для GKE Autopilot та керованих сервісів GCP:
- **Обчислення GKE Autopilot:**
  - `jobmatch-api` (2 репліки x 0.5 vCPU, 1 GB RAM):
    - CPU: $1.0\text{ vCPU} \times 730\text{ годин} \times \$0.0445/\text{vCPU-год} = \$32.49/\text{місяць}$
    - Memory: $2.0\text{ GB} \times 730\text{ годин} \times \$0.0049/\text{GB-год} = \$7.15/\text{місяць}$
  - `jobmatch-web` (2 репліки x 0.25 vCPU, 256 MB RAM):
    - CPU: $0.5\text{ vCPU} \times 730\text{ годин} \times \$0.0445/\text{vCPU-год} = \$16.24/\text{місяць}$
    - Memory: $0.5\text{ GB} \times 730\text{ годин} \times \$0.0049/\text{GB-год} = \$1.79/\text{місяць}$
  - `agentgateway` (Envoy) (2 репліки x 0.5 vCPU, 512 MB RAM):
    - CPU: $1.0\text{ vCPU} \times 730\text{ годин} \times \$0.0445/\text{vCPU-год} = \$32.49/\text{місяць}$
    - Memory: $1.0\text{ GB} \times 730\text{ годин} \times \$0.0049/\text{GB-год} = \$3.58/\text{місяць}$
  - **Проміжний підсумок GKE:** **~$93.74/місяць** (Примітка: фіксована плата за керування кластером у розмірі $73.00/місяць повністю покривається безкоштовним лімітом GKE).
- **GCP Memorystore for Redis (Basic Tier, 1 GB):**
  - $1\text{ GB} \times 730\text{ годин} \times \$0.0127/\text{год} = \mathbf{\$9.27/\text{місяць}}$
- **Google Cloud Load Balancer (GCLB):**
  - Базовий тариф: $730\text{ годин} \times \$0.025/\text{год} = \mathbf{\$18.25/\text{місяць}}$
- **Google Cloud Armor Standard + WAF правила:**
  - Вартість політик, правил та обсягу запитів: **~$7.00/місяць**
- **Вихідний трафік та хмарне сховище (Egress & Cloud Storage):**
  - Оцінка витрат на трафік та збереження логів: **~$10.00/місяць**
- **ЗАГАЛЬНІ ЩОМІСЯЧНІ ВИТРАТИ НА ІНФРАСТРУКТУРУ:** **~$138.26/місяць**

### Безпекове мапування сервісів GCP на захист від OWASP

| Сервіс GCP | Цільова вразливість OWASP | Деталі захисту (Mitigation) |
| :--- | :--- | :--- |
| **Google Cloud Armor** (WAF та DDoS) | **OWASP Web A03: Injection** (SQLi, XSS)<br>**OWASP Web A04: Insecure Design**<br>**OWASP LLM01: Prompt Injection** | • Набір правил WAF на базі OWASP ModSecurity Core Rule Set блокує SQLi, XSS та локальні/віддалені ін'єкції файлів на рівні межі мережі.<br>• Забезпечує обмеження частоти запитів (rate limiting) та захист від DDoS на рівні L7.<br>• Користувацькі правила фільтрації дозволяють блокувати специфічні ключові слова промпт-ін'єкцій до того, як вони потраплять у кластер. |
| **Google Kubernetes Engine (GKE) Autopilot** | **OWASP Web A01: Broken Access Control**<br>**OWASP Web A05: Security Misconfiguration** | • **Мережева ізоляція:** Вбудована підтримка NetworkPolicies (через Dataplane V2) дозволяє обмежувати трафік між namespace (наприклад, ізоляція `agentgateway-system` від `jobmatch-prod`).<br>• **Безпека вузлів:** Вузли заблоковані Google (відключено доступ по SSH, технологія Shielded VM, автоматичні оновлення та патчі ОС).<br>• **Безпека контейнерів:** Дозволяє роботу лише перевірених подів відповідно до Pod Security Standards (заборона запусків від імені root, обмеження монтування хосту). |
| **Google Secret Manager** (з Workload Identity) | **OWASP Web A02: Cryptographic Failures**<br>**OWASP LLM10: Model Theft / Excess Agency** | • Надійне шифрування API-ключів та паролів у стані спокою та під час передачі.<br>• **Workload Identity:** Асоціює Kubernetes Service Accounts безпосередньо з ролями GCP IAM, усуваючи потребу у збереженні довготривалих JSON-ключів сервісних акаунтів у контейнерах.<br>• Дозволяє інжектувати ключі *виключно* в проксі `agentgateway` у закритому просторі імен `agentgateway-system`, не розкриваючи їх подам бекенду чи логам. |
| **Artifact Registry** & **Container Analysis** | **OWASP Web A06: Vulnerable & Outdated Components** | • Автоматичне сканування образів на вразливості пакетів ОС (CVE) при кожному пуші контейнера.<br>• Сповіщення розробників та можливість блокування деплою образів із критичними CVE. |
| **Google Cloud Load Balancing (GCLB)** | **OWASP Web A02: Cryptographic Failures** | • Забезпечує термінацію SSL/TLS з використанням керованих Google сертифікатів.<br>• Застосовує сучасні політики SSL (підтримка виключно TLS 1.2 або TLS 1.3, відключення застарілих шифрів). |
| **Cloud Logging** & **Cloud Monitoring** | **OWASP Web A09: Security Logging & Monitoring Failures** | • Централізований збір логів GKE Audit Logs, VPC flow logs, Load Balancer access logs та логів доступу до Secret Manager.<br>• Налаштування алертів для сповіщення команди у разі підозрілих IAM-викликів чи невдалих спроб автентифікації. |
...
### Контекст
Платформа JobMatch використовує алгоритми семантичного пошуку для порівняння векторних ембеддінгів резюме та вакансій. Традиційне розгортання векторних баз даних (таких як Qdrant або pgvector) всередині кластерів Kubernetes вимагає налаштування стійкого збереження даних (Stateful Persistence). Стандартне керування станом хмарних баз даних (Persistent Volume Claims, мультизональна реплікація томів) призводить до вищих базових витрат (FinOps) та збільшує операційну складність адміністрування вузлів (Ops), що є неефективним для збереження векторних індексів пошукового кешу, які підлягають повторній реконструкції.

### Рішення
1. **Розділення стану:**
   - **Структурований стан застосунку:** Облікові записи користувачів, розібрані профілі та лог пошуків зберігаються у керованій реляційній базі даних (**GCP Cloud SQL для PostgreSQL**).
   - **Семантичний індекс пам'яті:** Розміщується всередині **бездержавного (ephemeral/stateless) Qdrant**, розгорнутого у GKE Autopilot. Стійкі дискові томи SSD (PVC) для збереження даних Qdrant не виділяються.
2. **Динамічне повторне індексування в пам'яті:**
   - Усі векторні ембеддінги в побі Qdrant розглядаються як тимчасовий кеш швидкого пошуку.
   - У разі перенесення подів Qdrant, горизонтального масштабування або перезапуску, вони ініціалізуються з порожнім індексом у пам'яті. API-сервер автоматично асинхронно запускає фоновий процес повторного індексування активних вакансій з PostgreSQL та первинних скраперів, відновлюючи повну працездатність векторного пошуку менш ніж за 60 секунд.

### Наслідки
- **Плюси:**
  - **Економія FinOps:** Нульова вартість за використання стійких мультизональних дискових сховищ SSD у хмарі, що зводить базові витрати на обчислення в GKE Autopilot до мінімуму.
  - **Простота Ops:** Відсутність необхідності керувати графіками резервного копіювання, протоколами реплікації даних або помилками підключення дискових томів для векторної бази даних.
  - **Висока масштабованість:** Бездержавні поди Qdrant масштабуються горизонтально або миттєво перезапускаються в будь-якій зоні доступності без затримок на монтування дисків.
- **Мінуси:**
  - Виникнення короткочасної затримки прогріву ("warm-up" period) при "холодному" старті подів, поки фонове завдання не завершить повну реконструкцію векторних індексів.
