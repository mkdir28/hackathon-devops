...
│   ├── finops-llm.md         # Детальний опис FinOps маршрутизації
│   ├── Tests.md              # Посібник з тестування
│   └── ProjectCodeMindMap.md # Мапа та огляд взаємозв'язків коду
```
...
* 📑 **[Architectural Decision Records (ADR)](doc/ADR.md)** — Чому саме так побудована система, FinOps аналіз, обґрунтування вибору моделей та розрахунки Unit-економіки.
* 📐 **[High-Level Solution Design (HLD)](doc/HLD.md)** — Схеми архітектури, життєвого циклу Агента та діаграми розгортання в Kubernetes.
* 🔄 **[CI/CD & GitOps Guide](doc/archive/cicd.md)** — Детальний опис налаштування FluxCD, тегування образів та автоматичного оновлення релізів.
* 🧪 **[Evaluation Engine Guide](doc/archive/eval.md)** — Опис налаштування контуру Evals, тестових сценаріїв та оцінки LLM-as-a-Judge.
* 💸 **[FinOps LLM Routing Guide](doc/archive/finops-llm.md)** — Детальний опис динамічної маршрутизації за типом завдання (x-gateway-task-name) та рекомендації щодо моделей.
* 🧪 **[Testing Guide](doc/archive/Tests.md)** — Посібник з локального тестування (Vitest, моки, покриття коду).
* 🗺️ **[Project Code Mind Map](doc/archive/ProjectCodeMindMap.md)** — Структура та огляд взаємозв'язків коду в проекті.